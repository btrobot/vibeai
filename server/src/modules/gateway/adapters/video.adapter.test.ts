...
describe('VideoAdapter', () => {
  ...
  describe('错误处理', () => {
    ...
    it('渠道 defaultParams.apiKey 提供凭证时，env 无 Token 也可调用（对齐 boli 渠道凭证解析）', async () => {
      ...
      const cfgModel = {
        ...mockModel,
        defaultParams: { ...mockModel.defaultParams, apiKey: 'cfg-video-key', baseUrl: 'https://cfg.example.com' },
      };
      const cfgAdapter = new VideoAdapter();

      await expect(cfgAdapter.execute({ prompt: 'a video' }, cfgModel, mockContext)).resolves.toBeDefined();
    });
  });
});
