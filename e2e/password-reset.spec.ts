...
test.describe('密码重置流程', () => {
  test('忘记密码页面加载', async ({ page }) => {
    ...
    // 应该显示标题和邮箱输入框（CardTitle 渲染为 div，非 heading role）
    await expect(page.getByText('忘记密码')).toBeVisible();
    await expect(page.locator('#email')).toBeVisible();
    await expect(page.getByRole('button', { name: '获取重置链接' })).toBeVisible();
  });
  ...
  test('未注册邮箱提交返回成功（防枚举）', async ({ page }) => {
    ...
    await page.getByRole('button', { name: '获取重置链接' }).click();
...
test.describe('重置密码页面', () => {
  test('重置密码页面加载', async ({ page }) => {
    ...
    // 标题「重置密码」与提交按钮文案重复，用唯一 description 断言页面渲染
    await expect(page.getByText('设置你的新密码')).toBeVisible();
    await expect(page.locator('#token')).toBeVisible();
    await expect(page.locator('#newPassword')).toBeVisible();
    await expect(page.locator('#confirmPassword')).toBeVisible();
  });

  test('URL 带 token 时隐藏令牌输入框（token 来自 URL）', async ({ page }) => {
    const testToken = 'test-reset-token-123';
    await page.goto(`/reset-password?token=${testToken}`);
    await page.waitForLoadState('load');

    // 新 UI 逻辑：URL 带 token 时隐藏令牌输入框，直接使用 URL 中的 token
